...
export const auth = {
    ...
    login: async (email, password) => {
        ...
        // ✅ حفظ التوكن بـ 'token' عشان يتوافق مع باقي الـ API calls
    ...
    signup: async (payload) => {
        ...
        // ✅ احفظي الـ identifier (رقم الجوال) عشان صفحة OTP تستخدمه
        if (raw?.identifier) {
    ...
    verifyOtp: async (identifier, code) => {
        ...
        if (raw?.next_step === 'verify_email') {
            // Pass token through without saving — page decides when to commit it
            ...
            return { next_step: 'verify_email', email: raw.email ?? '', token: pendingToken };
        }
    ...
    verifyEmailOtp: async (email, code, skip = false) => {
        const raw = await request('/api/auth/verify-email-otp', {
            method: 'POST',
    ...
    resendEmailOtp: async (email) => {
        return await request('/api/auth/send-email-otp', {
            method: 'POST',
