...
export const auth = {
    login: async (email, password) => {
        const raw = await request('/api/auth/login', {
            method: 'POST',
            body: JSON.stringify({ identifier: email, password }), // ✅ fixed: backend expects 'identifier'
        });
        ...
        // Login step 1: backend returns OTP confirmation (no token yet)
        if (!token && raw?.success && raw?.identifier) {
            return { otpRequired: true, identifier: raw.identifier, channel: raw.channel };
        }

        if (!token) throw new Error('No token received from server.');
        return { token, user: raw?.user ?? raw?.doctor ?? null };
    },
